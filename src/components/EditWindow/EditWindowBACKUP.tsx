...
export default class EditWindow extends React.Component<IProps, IState> {
    ...
    public render() {
        return (
            <div>
                ...
                maxWidth={"md"}
                fullWidth
                open={this.props.openEditWindow}
    ...
    private openWorkoutDialog = () => {
        if (this.props.workoutData.workoutId !== 0) {
            this.updateWorkoutContents()
        } else if (this.props.workoutData.workoutId === 0) {
        ...
        this.setState({workoutName: this.props.workoutData.workoutName})
        this.setState({workoutDescription: this.props.workoutData.workoutDescription})
    }

    // get request using filtering method to obtain all the exercises based on current workout 
    public updateWorkoutContents = () => {
        this.setState({exerciseData: this.props.workoutData.exercises})
    }
    ...
    private uploadWorkout = () => {
        ...
        if (this.props.workoutData.workoutId === 0) {
            ...
            fetch('https://fittracapisqlite.azurewebsites.net/api/Workouts/EditWorkout?id='+this.props.workoutData.workoutId, {
                body: JSON.stringify(workoutData),
                headers: {
                    Accept: "text/plain",
                    "Content-Type": "application/json-patch+json"},
